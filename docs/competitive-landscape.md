...
| Machine-checked accuracy evidence | 🟡 vendor QA, opaque | 🟡 | 🟡 vendor-audited | ✅ 2,679 tests, closed forms, bitwise dual-executor equivalence, parity harness, and checks against **published** figures (docs/sources/ — Mack's Taylor–Ashe reserves reproduced to the rounding he printed) |
...
| Grid / cross-machine scale-out | ✅ Enterprise grid | ✅ | ✅ GridLink | ✅ dispatch to remote engine instances (RFC-075), bitwise across workers that **attest the same arithmetic** — and refused otherwise, which no incumbent checks at all. Milestone M2: **24.0M inner policy-scenario cells in 103.1s**, and the same nested block split four ways is bitwise identical to the block run whole |
...
| Multi-tenant SaaS packaging | 🟡 Managed Cloud Service | 🟡 R³S cloud | ✅ | ✅ Dockerfile, compose profile and Helm chart, plus a tenancy model where **isolation is asserted by tests rather than described** (RFC-078): every run-scoped route answers the wrong tenant 404 not 403, because run ids are request fingerprints and a 403 is a membership oracle. Identical work from two tenants computes once, and the residual liveness signal that leaves is **published in words** with a switch to close it — no incumbent states its own multi-tenancy leak |
| SOC 2 / audit substrate | 🟡 vendor-attested | 🟡 | 🟡 | ✅ nineteen Trust Services criteria mapped to mechanisms, each naming a test the build re-verifies (RFC-079); the audit binder is a **regenerable section of the evidence pack**, not a document. Three rows say *not claimed* — a control map with no gaps is one nobody audited |
| Release cadence & regulatory calendar | ✅ quarterly, contractual | ✅ | ✅ | ✅ every dated regulation set carries a review date and is **never removed**, so a prior period still reproduces (RFC-080); a CI gate refuses a moved golden value without an expected-change note — the one review a code diff cannot perform |
| Pilot process | 🟡 consulting engagement | 🟡 | 🟡 | ✅ the whole client pilot executed end to end in CI against synthetic fixtures, with `--prove-it-bites` requiring the reconciliation to fail on one part in ten million (RFC-081) — the pilot has been run a thousand times before it is run once |
